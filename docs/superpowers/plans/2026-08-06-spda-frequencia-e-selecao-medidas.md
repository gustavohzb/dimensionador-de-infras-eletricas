...
A busca varre até 600 000 arranjos e leva ~2 s no pior caso, então **não pode
rodar no render**. Ela dispara num botão, e o painel tem quatro estados: nada
buscado ainda, buscando, resultado, e nada encontrado.
...
**Interfaces:**
- Consumes: `buscarMedidas` e `atendeNorma` de `src/lib/spdaBusca.js`, `cientifica` de `./formato`.
- `buscarMedidas(entrada)` devolve `{ combinacoes, avaliadas, esgotou, melhorParcial, semNg }`, onde cada combinação é `{ indices, esforco, escolhas, r1, r3, piorF, entrada }` e cada escolha é `{ eixo, label, esforco }`.
...
- [ ] **Step 3: Verificar (o controlador faz a parte de navegador)**

Rodar `npm test -- --run`, `npm run lint` e `npm run build`. O controlador confere no navegador:
1. o painel aparece com o botão "Procurar medidas" e nenhuma lista;
2. clicar mostra "Procurando…" e depois até três opções;
3. cada opção lista medidas e mostra R1, R3 e F resultantes;
4. clicar em "aplicar" muda os campos dos painéis Estrutura e Proteções, e o veredito passa a verde nos três cartões;
5. depois de aplicar, o painel some;
6. mexer num campo depois de buscar limpa o resultado e devolve o botão;
7. o console não acusa erro novo.
