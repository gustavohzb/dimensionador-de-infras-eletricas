...
import { ehTrifolioEspacado, verificarTrifolioEspacado } from "../lib/packing";
...
export default function InfraTab({ dark, pendingImport, onConsumeImport }) {
  ...
  // Trifólio 2D: o feixe entra numa fileira com vão livre de 2× o diâmetro. O
  // espaçamento é de cada trifólio (botão próprio), não do trecho. O botão fica
  // bloqueado onde o arranjo não existe: eletroduto é feixe confinado, e cabo
  // de comando no trecho significa septo divisor, com outra geometria.
  const ehEletroduto = dim.kind === "duct";
  const temComando = cables.some((c) => c.type === "comando");
  let motivoSemEspacado = null;
  if (temComando) motivoSemEspacado = "Trecho com cabo de comando usa septo divisor — a fileira espaçada não se aplica.";
  else if (mode === "verificar" && ehEletroduto) motivoSemEspacado = "Eletroduto é feixe confinado — a fileira espaçada não se aplica.";
  const tem2D = cables.some(ehTrifolioEspacado);
  // 2D no trecho mas num lugar onde o arranjo não vale (trocou para eletroduto
  // depois de adicionar, por exemplo): o desenho cai para o feixe comum, e a
  // tela diz isso em vez de deixar o usuário achando que está espaçado.
  const avisoSem2D = tem2D && motivoSemEspacado ? motivoSemEspacado : null;
  const verif2D = tem2D && !motivoSemEspacado ? verificarTrifolioEspacado(cables, trayWidth, trayHeight) : null;
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-[340px_1fr]">
        <section className="space-y-3">
          ...
          <div className={cardCls}>
            ...
            {catalogo === "forca" ? (
              <CableForm onAddCable={addCable} onAddTrifolio={addTrifolio} motivoSem2D={motivoSemEspacado} />
            ) : (
            ...
            <div className={cardCls}>
              ...
              {avisoSem2D && (
                <p className="mt-2 text-xs font-medium text-amber-600 dark:text-amber-400">
                  Há trifólio 2D no trecho, mas {avisoSem2D.charAt(0).toLowerCase() + avisoSem2D.slice(1)} Os trifólios 2D aparecem como feixe comum.
                </p>
              )}
              {verif2D && (
                <p
                  className={`mt-2 text-xs ${
                    verif2D.cabe ? "text-slate-500 dark:text-slate-400" : "font-medium text-red-600 dark:text-red-400"
                  }`}
                >
                  Fileira espaçada 2D: ocupa <b>{Math.round(verif2D.fileira)} mm</b>
                  {verif2D.temResto && ", e os demais cabos vão à direita dela"}.{" "}
                  {verif2D.cabe && `Cabe nos ${trayWidth} mm da infraestrutura.`}
                  {!verif2D.cabe && verif2D.faltam !== null && `Não cabe em ${trayWidth} mm — faltam ${Math.ceil(verif2D.faltam)} mm.`}
                  {!verif2D.cabe && verif2D.faltam === null && `O trecho não cabe em ${trayWidth}×${trayHeight} mm.`}
                </p>
              )}
            </div>
